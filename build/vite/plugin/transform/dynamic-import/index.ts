...
const dynamicImportTransform = function (): Transform {
    return {
        test({ path }) {
            ...
            return path.includes('/src/utils/helper/dynamicImport.ts') || path.includes(`\\src\\utils\\helper\\dynamicImport.ts`)
        },
        transform({ code }) {
        ...
        export default function (id) {
           switch (id) {
          ${files.map((p) => `   case '${getPath(p)}': return  () => import('${p.replace('src/views', '/@/views').replace(/\/\//g, '/')}');`.replace('.tsx', '')).join('\n  ')}
